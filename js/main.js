jQuery(document).ready(function($){
	//variables
	var hijacking= $('div').data('hijacking'),
		animationType = $('div').data('animation'),
		delta = 0,
        scrollThreshold = 5,
        actual = 1,
        animating = false;
    ...
    function unbindScroll(section, time) {
    	...
    	if( hijacking == 'off') {
    		$(window).off('scroll', scrollAnimation);
    		( animationType == 'catch') ? $('div, html').scrollTop(section.offset().top) : section.velocity("scroll", { duration: time });
    	}
    }
